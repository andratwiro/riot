...
| `map.js` | The affinity map (reveal-only): party vote-distances flattened to 2D, user placement, animated entrance. **Eight projections** (classical MDS default; PCA, CA, kernel PCA, SMACOF, Sammon, spectral, t-SNE — all vanilla JS over the shared Jacobi eigensolver), picked from the `#mapProj` chip row under the map; each is Procrustes-aligned to the MDS baseline so switching morphs the dots in place instead of spinning the map. Presence always publishes the CANONICAL MDS position (`publishCoord`), independent of the local pick. In live sessions every participant — synthetic voters included — is blended onto the current projection from their full cast-marker ballot record (`LIVE.peerVotes` → `blendCoord`), peer dots keyed by pid so they morph too; outside live, a peer sits where it published. |
