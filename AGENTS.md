...
| `roomfloor.js` | **THE FOOTER ROOM** (live sessions only; loads after `live.js`). The room's people as one organic crowd at the bottom of the booth (`#roomfloor`), replacing the top strip and the in-card faces (Rob, 2026-06-13 — see `docs/FOOTER_ROOM.md`). A light force-sim (`window.RF`): attraction to a per-body target, neighbour repulsion with ~14% overlap tolerated (the "busy, slightly colliding" feel), gentle jitter, damping; transforms/opacity only (holds 60fps to ~40 bodies). Two states bound to the current card, mirroring the card body above — `RF.cluster()` (a loose centred blob while a ballot is open) and `RF.piles(id)` (on reveal the same bodies fan to the sides into Against/Abstain/For under the card's columns, raining in live as casts arrive via `LIVE.peerVotes`; non-voters sink to the screen bottom and peep ~40% over the edge, clipped; mine from `answers`). `RF.regroup()` is the cluster again on the next card. The crowd fills down to the foot **level with the floating wave hand** (v1.21 — no longer a reserved empty band above it / two rows); a clearance ring keeps the hand's own little clearing in BOTH states — NO emoji piles under it (v1.22; the wave button's background is now `transparent`, so any stray body would otherwise show its square). `place()` reads the hand's actual rendered centre (`measureHand` — the button is `position:fixed`, so test the rect, NOT `offsetParent`, which is always null for fixed) and pushes any body whose CENTRE is inside `HAND_CLEAR + its radius` straight out to the ring edge; a bounded RENDER offset (not a velocity force, so it can't accumulate or fight the cluster↔pile reorg) that is 0 outside the ring, so it stays continuous; sparse cards leave the hand clear and nothing moves. The decided crowd also floats a `BOTTOM_GAP` (20px) strip above the absolute bottom so a fresh vote visibly CROSSES that strip into its pile (esp. abstain, the centre column under the hand), instead of resting flush against the edge where the move is lost; and directly under the hand the resting floor lifts to the hand's centre line (`yhi` — only when `|b.x−handX| < HAND_CLEAR+r`), so an under-hand body floats up and the ring then spreads it sideways: the crowd packs around and above the hand, never into the strip directly below it (v1.23). Non-voters are the exception — they sink past the bottom edge and peep, unclamped. Size-by-count reuses the lobby's step (big when few, tight at a full house); your body wears the violet ME ring, `RF.wave(pid)` gives it an upward impulse (from `bounceFace`), `RF.tick(pid)` a quiet pop on a cast (from `activityTick`). Gated entirely on `body.live-floor` (set by `live.js` `liveFloor()` only for a seated voter mid-card — voting/paused/reveal); inert in async rooms, solo, the lobby, the final reveal, and for the moderator (`RF.active()` is false), so async/solo/stage are untouched. `prefers-reduced-motion` places bodies at their targets with no loop. |
