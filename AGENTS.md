...
| `views.js` | Secondary views: the minutes (raw-data log, one tap via the header `§`; a page-wide provenance pill toggles every title between the AI-reworded headline and the source's own wording), party comparison, **curator mode** (moderator-only: `?role=moderator` reveals a Curator switch on the Minutes page — review checks on each row + marks-JSON download; a persisted dev flag never activates on a plain voter URL), votes export/import, options sheet. |
