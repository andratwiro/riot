...
| Path | What it is |
|------|------------|
| `index.html` | The shared viewer (~vanilla HTML/CSS/JS): Polis-style affinity bar, 2D opinion map, decision card-stack, city switcher. A `?city=reus\|brussels` loader picks which `cities/<id>/` bundle to load; only data + config differ per city. User votes go to `localStorage` (never committed). Carries the visible **version tag** (see Conventions). |
