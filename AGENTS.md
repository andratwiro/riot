...
jurisdiction-agnostic and now runs **eight instances** behind one shared viewer:
...
- **Bundestag** (`?city=bundestag`) — the sitting German chamber, 21.
  Wahlperiode: ten namentliche Abstimmungen 2025–2026 (Mietwucher, Wehrdienst,
  Rentenpaket, § 188 StGB, GEAS, Grundsicherung, Energiesteuer, Stromsteuer,
  BAföG-Beschlussempfehlung, Jahresemissionsmengen). NOT demo-grade — the
  Commons posture on the official record: `scripts/fetch_votes_bt.py` parses
  the Bundestag's per-vote XLSX (stdlib zip+XML) into
  `data/bundestag/rollcalls/` (committed per-MP audit trail, independently
  cross-checked against the abgeordnetenwatch.de API v2, CC0; a >1-member
  per-Fraktion mismatch is fatal, a 1-member one is recorded in the JSON) and
  `build_table_bt.py` **computes** each Fraktion's direction from cast votes —
  three-way plurality over ja/nein/Enthaltung (the Bundestag has a REAL
  abstain button and whole Fraktionen use it; ties = abstain; fraktionslose
  never aggregate). Fraktionen: CDU/CSU, AfD, SPD, Grüne, Linke. The first
  all-GERMAN instance — card copy and chrome strings both (`lang`/`srcLang`
...
| `live.js` | **LIVE SESSION** — lockstep room voting (lobby → per-card voting/reveal → final reveal; everyone on the same card). State machine in `rooms/<city>/live` (RTDB), written ONLY by the moderator client (`?role=moderator`, whose screen doubles as the projector/stage view). Voters write an anonymous tally increment + a `cast` marker carrying their direction (reveal-only; see Conventions); countdown renders from a server-clock deadline (`.info/serverTimeOffset`); timeouts are recorded nowhere and so are auto-excluded from affinity. Per-card reveal: the chamber's official stamp first — carrying the real margin as a `.st-m` subtitle (`marginPair`/`chamberMargin`: winner-first head-count from `d.tally`, `· N abstained` only when nonzero; the bicameral Congress deck ships `tally_house`/`tally_senate` and renders both chamber pairs; unanimous actas ship an all-zero `tally` ⇒ the subtitle says "unanimous", never a fabricated 0–0; no tally data at all ⇒ verdict alone, never a guess) — then, on the voter's phone, the **chamber only** (Rob, 2026-06-13, `renderVoterChamber`): each party's `party_votes_canon` direction drops its `logoEl` disc on the Against/Abstain/For columns, NO emoji faces in the card (the room's one-line verdict — `verdictCopy`/`.lv-verdict`, "the room agrees / would have decided differently" — stays on the card). The room itself left the card: it now lives in the **footer room** (`roomfloor.js`, see its row) which fans the crowd into the same three columns BELOW the card. (The moderator STAGE keeps the combined room+chamber piles enlarged — `renderLivePiles` — the projector wants the crowd on the big screen.) Absent tokens (single-chamber measures, the GHOST) don't land; affinity/map detail stays in the final reveal/minutes. The final reveal's room-v-chamber rows carry the same margin format on BOTH badges (room side from `LIVE.tally`) so they read as parallel facts. The voter lobby is **the seated waiting room, glanceable** (Rob, 2026-06-12: nobody reads — its only jobs are "something is about to happen / people are arriving / wait for it"; the shared URL gates FIRST): the app's own header (left-aligned, as everywhere), a small head (pulsing live chip ~1.6s, Archivo 800 title) over **the avatars as the hero** (Rob, 2026-06-13): the gathered faces fill the centre — EVERYONE shows, no 8-cap and no `+N` chip, and the cluster scales down as the seats fill (big when few, tight at a full house — `lobbyPresence` sets `--av/--ov/--fs` per render), overlap + the newcomer pop kept; the big mono count sits beneath with `aria-live="polite"` counting peers + me — never "0 in the room" to the person standing in it. All copy from `CFG.lobby`; **who sits here** (Rob, 2026-06-13) is one quiet `lb-who` trigger under the count that opens a **bottom sheet** (`#rosterSheet`, same bones as the options `#sheet` but riding ABOVE the wave hand at z-50, with pull-down-to-dismiss + scrim/Escape) listing the groups in this sitting as identity rows (disc + name + a line from the party table's `blurb`, the same rows + `parties_label` the solo cover uses; identity ONLY, never a deck stance — the firewall holds pre-vote). The sheet (NOT inline) keeps the avatar hero unsquished and the glance a clean centred stack; the old dry session-metadata (institution / docket-count / disclosure lines) is gone; the old intro paragraph is gone and the privacy line moved to the FIRST vote card (`LIVE.cardNotes` → `.acts-priv` micro-type under the vote buttons, rendered by app.js, dead after the first cast; a blind-vote rule line rode along briefly — Rob cut it 2026-06-12). On open (`#lobby.counting`) the room dims and a big **3·2·1 counts the sitting in** over the faded faces — the chair's formula (`sittingOpenedFormula`) sits small above the number — then the first card lands. **Seats (Rob, 2026-06): the voter URL is the sitting's entrance and nothing else.** A voter boot holds clean paper (`html.lvhold`, every voter URL) until the store answers `current`: no sitting ⇒ **the wall** (`lvWall` — the holding page, reversible: a sitting created later turns it back into the gate); a sitting whose sid matches the tab's `riot.sid.v1` ⇒ silent re-seat (`mpJoin`, no gate — a pull-to-refresh doesn't eject anyone); anything else ⇒ the **seat gate** (`showSeatGate` → app.js `gateShow(true)`, face preselected, button label from `CFG.lobby.cta` — Reus "Ocupa el teu seient"; Rob, 2026-06-12: the gate triggers on the shared URL itself, whatever the sitting's state — a lobby-first variant was rolled back same day, un-seated watchers can't see each other so the gathering read as broken). The TAP is the join (`LIVE.seat`): it stores the sid, clears local `answers` (a stale ballot from an earlier sitting would block voting and fake "in flight" casts), publishes presence stamped `s:<sid>`, and lands the tab wherever the sitting is — lobby, the live card, or the final reveal (dotless there: no cast markers). `voterCount`/lobby faces/stage faces/`checkAllIn` all see only this sitting's records (`mpVisiblePids`), so stale tabs neither inflate the room nor stall the auto-advance; ending a sitting walls every non-final tab (`liveEnded` → `mpLeave`), a NEW sid re-gates everyone (one tap back in), and the solo booth is unreachable on a backend — only `?simroom`/stubbed-Firebase dev runs still get it, plus the one deliberate exception: `?solo=1` (see app.js) skips live resolution and the room entirely. **Lobby refresh hold:** while seated in the lobby, a sessionStorage flag (`riot.liveLobby` = city id) re-paints the CFG-prefilled lobby on frame one while Firebase reconnects. **Synthetic voters:** `cfg.bots` (moderator setup option, "synthetic voters", 0–24) seats N fake people in the room to stage how a fuller session looks — real participant records + random tally/cast writes from the moderator's client (presence via `PEERS` in sim rooms / `mpPart` on Firebase; casts via `lvStore` either way), so every phone renders them exactly like humans: lobby faces, ballots-in counts, reveal piles. Human-ish timing (spread + deadline cluster + the odd timeout), deterministic pids (`bot<i>`) so a reloaded moderator re-adopts them, `onDisconnect` + session end remove them. **Ghost mode:** `cfg.ai` (moderator setup switch, "GHOST" — shown only when the city ships `AI_VOTES`; the wire key predates the name) seats the GHOST as one more row in the reveal for the WHOLE room: every client mirrors the session flag on each snapshot (`liveSyncGhost` → `applyGhostParty`), a voter parked on the final screen keeps it, and back-to-booth resets it — the old per-visitor ⚙ toggle is gone (its `riot.ai.v1` key is purged at load). **The stage join moment:** the lobby stage shows a scannable QR (`qrSVG` — inline ink SVG via the vendored `assets/qrcode.min.js`) above the join URL, and that URL ALWAYS carries `?city=<id>` — even for Reus — because the bare URL is the holding page, not an entrance. Voters arrive over presence, not session writes, so the `renderStrip` wrapper also re-renders the moderator's stage while the state is `lobby` (without it the gathering faces never refresh — nothing touches the session node until the sitting opens). |
| `roomfloor.js` | **THE FOOTER ROOM** (live sessions only; loads after `live.js`). The room's people as one organic crowd at the bottom of the booth (`#roomfloor`), replacing the top strip and the in-card faces (Rob, 2026-06-13 — see `docs/FOOTER_ROOM.md`). A light force-sim (`window.RF`): attraction to a per-body target, neighbour repulsion with ~14% overlap tolerated (the "busy, slightly colliding" feel), gentle jitter, damping; transforms/opacity only (holds 60fps to ~40 bodies). Two states bound to the current card, mirroring the card body above — `RF.cluster()` (a loose centred blob while a ballot is open) and `RF.piles(id)` (on reveal the same bodies fan to the sides into Against/Abstain/For under the card's columns, raining in live as casts arrive via `LIVE.peerVotes`; non-voters sink to the screen bottom and peep ~40% over the edge, clipped; mine from `answers`). `RF.regroup()` is the cluster again on the next card. The crowd lives down at the foot with the floating wave hand (no top strip / in-card faces; not two stacked rows). The hand sits LOW (CSS `bottom:11px`, `background:transparent`) and the decided crowd rests on a FLAT floor a `BOTTOM_GAP` (36px) above the absolute bottom — well above the hand — so only the hand's TOP pokes into the lowest row: nothing rests below the hand, a fresh vote visibly crosses the strip up into its pile, and few bodies ever meet the hand (which is what keeps the piling calm). Two coupled rules keep it smooth (v1.24, tuned by jitter measurement — see `docs/FOOTER_ROOM.md`): (1) the floor is SOFT contact — `b.y` eases onto it and downward velocity is damped, never a hard clamp (the hard clamp's yank, plus an earlier per-column floor STEP, was the visible jitter); (2) `place()` keeps the hand legible by lifting any nearby body straight UP out of its clearing (`measureHand` reads the hand's real centre — it's `position:fixed`, so test the rect NOT `offsetParent`, always null for fixed; lift = `HAND_CLEAR(8)+r − d`, a bounded RENDER offset, **vertical only** — a sideways push flips direction with jitter and makes bodies orbit, vertical can't). Non-voters sink past the bottom edge and peep ~40% over it, clipped (unclamped). Earlier iterations (v1.21 same-level merge, v1.22 transparent bg + radial ring, v1.23 20px strip + per-column under-hand lift) are superseded by this calmer hand-low/floor-high layout. **Host resize (v1.26):** the floor and the hand are both at fixed SCREEN positions (hand is `position:fixed`; floor = `screenBottom − BOTTOM_GAP − r`, H cancels), so when the reveal grows the card and the footer host shrinks, only the bodies' LOCAL `b.y` go stale — they'd appear to drop below the hand. `measure()` shifts every body by `ΔH` on a host-height change, keeping the crowd at the same screen point (aligned with the hand, pile shape intact, invisible). This was the "emoji ignore the hand and pile below it when the stamp lands" glitch. Size-by-count reuses the lobby's step (big when few, tight at a full house); your body wears the violet ME ring, `RF.wave(pid)` gives it an upward impulse (from `bounceFace`), `RF.tick(pid)` a quiet pop on a cast (from `activityTick`). Gated entirely on `body.live-floor` (set by `live.js` `liveFloor()` only for a seated voter mid-card — voting/paused/reveal); inert in async rooms, solo, the lobby, the final reveal, and for the moderator (`RF.active()` is false), so async/solo/stage are untouched. `prefers-reduced-motion` places bodies at their targets with no loop. **Firefox tremble (v1.28):** Firefox re-rasterizes an emoji glyph whenever its sub-pixel offset shifts, so the per-frame jitter read as a harsh tremble (Chrome/mobile composite the glyph on a GPU layer and slide it smoothly). `place()` snaps the rendered translate to whole pixels on Firefox only (`IS_FF` UA test) — a resting body pins to one pixel (nothing to re-raster), motion >1px still animates; other engines keep the untouched sub-pixel path. |
| `cities/<id>/config.js` | Per-city chrome + tunables: `window.CITY_CONFIG` (name, logo, document `lang`, source-document `srcLang` for the original-wording toggle, `chamber` for live-session copy, `lobby` — live-lobby strings: live_chip / title / count_line / cta / sittingOpenedFormula (the countdown's small label), plus privacyLine which renders on the FIRST vote card, not the lobby; the lobby's opt-in roster borrows `solo_lobby.parties_label` for its toggle and the party table's `blurb`s for its rows — so the old metadata keys (one_liner / body_name / about_label / docketInstitutionLine / docketCountLine / disclosure) are no longer rendered (Rob, 2026-06-13: the dry metadata was kicked for the party roster) and survive only as dead config; Reus is the Catalan reference, Brussels/Congress/Commons/Weimar wording carries TODO(rob) markers, `mapGate`; `solo_lobby` — the SOLO cover's strings: kicker / title / lore[] / meta / cta / note, `{n}` = deck length — the history decks carry their process lore here). |
...
| `data/bundestag/` | The Bundestag layer: `rollcalls/*.json` (official per-MP records parsed from the Bundestag's XLSX ballots, committed audit trail — re-fetchable via `fetch_votes_bt.py`; the raw `.xlsx` cache is gitignored), `research_parts/*.json` (the verification agents' sourced fact base), `cards.json` (authored German copy + `verification` blocks; party directions/tallies/outcomes are NOT here — `build_table_bt.py` computes them from the rollcalls). |
| `assets/logos/` | Party/group logos + brand assets. The eight city marks (`reus_rose_color`, `brussels_iris`, `us_capitol`, `za_protea`, `tn_crescent`, `uk_portcullis`, `de_eagle`, `de_dome` — the Reichstag's glass dome on federal blue: the SITTING German chamber's mark; Weimar's gold eagle stays the memorial's) double as per-city favicons: app.js swaps the head's `#favicon` link to `CFG.logo` and sets `document.title = CFG.title` when a city boots — titles speak each instance's own language (Rob, 2026-06-12): "Ajuntament de Reus" / "Parlement bruxellois · Brussels Parlement" / "US Congress" / "Parliament of South Africa" / "البرلمان التونسي 2014–2018" / "House of Commons · 2019" / "Reichstag 1929–1933" / "Deutscher Bundestag" (the history decks' tab titles wear their years, era-stamp convention). |
...
| `Riot.md` | Vision/non-negotiables + phased build plan. |
...
```bash
# Real pipeline, Commons posture. Fetch the official per-vote XLSX ballots,
# parse them (stdlib) into the committed per-MP JSON, cross-check every
# per-Fraktion count against the abgeordnetenwatch.de API (CC0).
python3 scripts/fetch_votes_bt.py    # bundestag.de XLSX → data/bundestag/rollcalls/
# COMPUTE Fraktion directions (3-way plurality: ja/nein/Enthaltung) + merge
# the authored German copy.
python3 scripts/build_table_bt.py    # rollcalls + data/bundestag/cards.json → cities/bundestag/data.js
```
...
`cards.json` re-run its `build_table_us/za/tn.py`; after any change to
`data/bundestag/` re-run `build_table_bt.py` — the committed JSON and the
...
- **Frontend:** vanilla HTML/CSS/JS, no framework, no build step;
  ...
  then `roomfloor.js` after `live.js`; the boot block calls `liveInit()` after
  `mpInit()`.) Parallel agents should
